    ...
    >
      <scroll-view scroll-y class="pageContainer">
        <view class="contentItem" v-for="(item,index) in mineList" :key="index">
          <image
            class="itemBg"
            src="http://www.mingtongxa.com/wanzhuan/quanbaoBanner.png"
            mode="widthFix"
          ></image>
          <view class="header">
          ...
          <view class="content">
            <view class="contLeft">
              <view style="white-space:nowrap">{{item.shopName}}</view>
            ...
            <view class="contRight" @click="gotoUse(item.voucherCode,item.status,item.startTime,item.endTime,item.price,item.amount,item.shopName,item.shopDetail.imgUrl)">
              <!-- <view class="contRiLeft">
                <image
                  src="http://www.mingtongxa.com/wanzhuan/shengyu.png"
                  mode="widthFix"
                ></image>
                <text>剩余<text style="color:#FB0A16">20</text> 张</text>
              </view> -->
...
export default {
  ...
  methods: {
    getMineList(){
        this.$api.getMineOrList().then((res) => {
        if (res.code == 200) {
    ...
    gotoUse(num,status,startTime,endTime,price,amount,name,img){
      uni.navigateTo({
            url:
              "/subpages/pages/quanCode?num=" +
              num +
              "&status=" +
              status +
              "&start=" +
