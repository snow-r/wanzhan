<template>
  <View>
    <TopBar background="#fff" barTitle="注册" :back="true" />
        ...
        <view class="form">
          ...
          <view class="genderWrap">
            <view class="genderLabel">
              <image src="@/static/images/gender.png" mode="widthFix" />
              <text>请选择您的性别</text>
            </view>
            <view class="genderRadio">
              <view class="radioItemWrap" @click="chooseGender(1)">
                <image src="@/static/images/checked.png" v-if="gender == 1" />
                <image src="@/static/images/notChecked.png" v-else />
                <text>男</text>
              </view>
              <view class="radioItemWrap" @click="chooseGender(2)">
                <image src="@/static/images/checked.png" v-if="gender == 2"  />
                <image src="@/static/images/notChecked.png" v-else  />
                <text>女</text>
              </view>
            </view>
          ...
          <view class="toLoginWrap">
            <text>已有账号，</text>
            <text class="pageToWrap" @click="pageBack">速去登录</text>
    ...
    confirmRegister () {
      ...
      if (flag) {
        ...
        if(uni.getStorageSync('shareCode')){
          data['shareCode'] = uni.getStorageSync('shareCode')
        }
        uni.showLoading({mask: true})
        this.$api.comfirmRegister(sendData).then(res => {
          uni.hideLoading()
          if (res.code == 1) {
            uni.showToast({
              title: res.obj,
