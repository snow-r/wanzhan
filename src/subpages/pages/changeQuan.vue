      ...
      <scroll-view scroll-y class="pageContainer">
        ...
        <view class="address">
          ...
          <view class="addressRight">
            ...
            >
              <view slot="pCon" class="changeTime" v-if="times == ''">
                <text>请选择结束时间</text>
              </view>
              <view
                slot="pCon"
  ...
  methods: {
    ...
    getMoney() {
      ...
      this.$api.getMoneyDetail(sendData).then((res) => {
        ...
        this.zhifuPrice = res.data.price;
        this.yuanjia = res.data.amount;
        this.time = res.data.startTime;
        this.times = res.data.endTime;
        let oreder = res.data.useDayRule.split(",");
        ...
        this.startTime = res.data.shopDetail.beginTime;
        this.endTime = res.data.shopDetail.endTime;
        this.number = res.data.total;
        this.bukeshiyong = res.data.notUsable;
        this.shiyongfw = res.data.useScope;
        this.rulerList = res.data.useRule;
        this.totalOrder = res.data.total;
        this.saleNumber = res.data.saleNum;
      });
    },
    clearRule() {
      this.rulerList = "";
      ...
      if (this.useShiyongS) {
        ...
        if (
          ...
          this.useTimeShow =
            ...
            // this.endTime +
            // "," +
            this.daytypeOptions.find((a) => a.dictValue == useDayss[0])
              .dictLabel +
            "至" +
            this.daytypeOptions.find(
              (a) => a.dictValue == useDayss[useDayss.length - 1]
            ).dictLabel;
    ...
    gotoAdd() {
      ...
      let sendData = {
        ...
        saleNum: this.saleNumber,
        total: this.totalOrder,
      };
      this.$api.changeDaijin(sendData).then((res) => {
        uni.hideLoading();
        if (res.code == 200) {
          uni.showToast({
            title: "更改成功",
      ...
      uni.showModal({
        ...
        success: function(res) {
          if (res.confirm) {
            ...
            uni.showLoading();
            that.$api.deleteDaijin(sendData).then((ress) => {
              uni.hideLoading();
              if (ress.code == 200) {
                uni.showToast({
                  title: "删除成功",
